...
class SearchAdapter(val items: MutableList<ImageInfo>) : RecyclerView.Adapter<SearchAdapter.ViewHolder>() {
    ...
    class ViewHolder(val binding: RecyclerviewItemBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(item: ImageInfo) {
            Glide.with(itemView).load(item.thumbnail_url).into(binding.ivThumbnail)
